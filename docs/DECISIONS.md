...
### 2026-09-23 — Scripted variant: classify every turn, including answers to a pending question

- **Decision:** Every turn now starts at `route_intent`. Before, a pending entity question (`awaiting_field`) sent the next message straight to `capture_entity`, skipping classification. The classifier gets a fourth label, `answer`. It is offered only while a question is pending, and the prompt then includes that question. `answer` goes to `capture_entity`, which still stores the whole reply word for word. A search or an unclear message mid-consultation is handled as such, and `awaiting_field` stays set: the search reply ends with the pending question, and the fallback repeats it. A consultation request mid-consultation re-asks it.
- **Why:** Skipping classification made the variant's worst failure the most common one: any detour during the 4 questions, such as "show me the Telegram products", was silently saved as the entity. It also gave the graph two entry points. One router at the top is simpler to read, and the variant is still scripted where it counts: fixed question order, no extraction, template replies, fixed branches, no tools. In a live check with gpt-4o-mini, 13 of 13 sample messages got the right label, including one-word answers ("تهران", "B2C") and a bare "تهران" with no question pending (`unclear`).
- **Cost:** One classification call on answer turns, which used to make none. A real answer mislabeled as `unclear` gets its question asked again, which is milder than saving a detour as the entity.
- **Alternatives considered:** keeping word-for-word capture while a question is pending, as the documented limitation of the control baseline (rejected: it made the baseline worse than simple, and the comparison with the agentic variant doesn't rely on it); extracting the value from the answer with an LLM (that is the agentic variant's job, and would blur the difference between the two).

### 2026-09-23 — Scripted variant: take the entities a consultation request already states

- **Decision:** The `route_intent` call now also returns the 4 entities, each only if the message explicitly states it (`stated_entities`). On an `answer` or `consultation` turn, `capture_entity` fills every still-unset field from them, and `ask_entity` asks only for the fields still missing, in the fixed order. A consultation request stating all 4 goes straight to the analysis and suggestion. An `answer` that states no entity is still stored word for word as the pending field. This supersedes the word-for-word capture in the entry above, which covered only answers.
- **Why:** The variant asked all 4 questions even when the user had just given every answer in the request ("I run a cafe in Tehran, my customers are ordinary people, I sell on Instagram — advise me"). Asking for information the user already gave is a failure the task's consultation flow shouldn't have, not a useful contrast with the agentic variant. It stays one LLM call per turn, since the extraction rides on the existing classification call. In a live gpt-4o-mini check, 12 of 12 sample messages got the right label and values, including "مردم عادی" → B2C and an answer that also gave the city.
- **What stays scripted:** questions in the fixed order, template replies, fixed branches, no tools. Entities count only on an `answer` or `consultation` turn (not in a search), and a field already set is never overwritten, so there are still no corrections. The agentic variant extracts on every turn, overwrites on correction and re-runs the consultation.
- **Alternatives considered:** a separate extraction call (a second LLM call per turn for the same message); extracting on every turn, as the agentic variant does (a search like "site design for a cafe" would set the business type); allowing overwrites (that is the agentic variant's correction handling, and would need its re-run logic too).

### 2026-09-24 — Hybrid is the default search strategy

- **Decision:** `CONSULTANT_BOT_SEARCH_STRATEGY` now defaults to `hybrid` instead of `filter`. The other three strategies stay selectable.
- **Why:** Hybrid gives the best rankings of the four in `scripts/compare_search.py` (see the hybrid entry above), and its blended relevance floor is the strictest on off-catalog queries, so the demo should run on it out of the box. `filter` was the default only because it was built first.
- **Cost:** The first run downloads the embedding model (about 118 MB), and every startup imports `sentence-transformers`/`torch`. Setting `filter` or `tfidf` avoids both. The graph-wiring tests swap in the filter strategy so the suite doesn't load the model.
- **Alternatives considered:** keeping `filter` as the default (fast startup, but the weakest search); `embedding` alone (same cost as hybrid, looser on off-catalog queries).
