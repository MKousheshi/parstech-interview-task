...
- **Scope:** Flexible variant only. The rigid variant keeps `last_search_results`, which fits its scripted, template-driven follow-up handling.
...
### 2026-09-23 — Flexible variant: the entity extractor sees only the latest exchange
...
### 2026-09-23 — Flexible variant: the `search_products` tool applies the relevance threshold too
...
- **Scope:** Flexible variant only. The rigid variant still writes up whatever `top_k` returns, as its architecture doc says. The `filter` strategy's threshold is 0, so its results don't change.
