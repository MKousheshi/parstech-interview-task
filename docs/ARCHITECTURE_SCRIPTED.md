...
This variant implements the task spec close to literally, as an explicit state machine: entities are collected one at a time, in the exact order the spec gives (business type → customer type → location → sales channel), by tracking exactly one "awaiting" field in state at a time. Every turn goes through a single intent classifier with a fixed branch per label: an answer to the pending question, product search, consultation, or anything else, which lands in one canned fallback reply. An answer is stored word for word, with no extraction or reinterpretation. LLM calls are used only where the task requires judgment (intent classification, the two consultation-analysis calls) — search formatting and entity capture are pure template/deterministic code, not LLM-authored.
...
flowchart TD
    ...
    route -->|answer, while a field is pending| capture[capture_entity: store reply verbatim]
...
- **Answers are stored word for word** — a full sentence becomes the entity value as is, with nothing extracted from it, and only the pending field can be filled: volunteering the location while asked for the business type doesn't record it.
