...
- **Off-topic or unexpected input** — always gets the same canned fallback (or, worse, gets silently absorbed as the literal value of whatever entity is pending), never a genuinely on-point reply. This is the sharpest contrast with the agentic variant's design goal.
