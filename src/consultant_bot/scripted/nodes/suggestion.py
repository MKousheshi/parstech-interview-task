...
predictable, but weaker than the agentic variant's LLM-formulated query when the entities don't
use the catalog's vocabulary. Whatever the strategy's top-`k` returns is written up, with no
relevance threshold (the agentic variant's fix, deliberately left out here); only a completely
